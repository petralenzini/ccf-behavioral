...
cat_filescores = os.path.join(store_space,
     'CatToolBoxEndpointFiles_Scores.csv')
cat_fileraw = os.path.join(store_space, 'CatToolBoxEndpointFiles_Data.csv')
cat_fileregistr = os.path.join(store_space,
     'CatToolBoxEndpointFiles_Registration.csv')
...
files4process = files4process.loc[files4process.raw_cat_date.isnull(
) == True].copy()
