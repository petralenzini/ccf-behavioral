...
store_space = os.path.join(root_store, 'penncnp') #this will be the place to save any snapshots on the nrg servers
try: 
...
#WU,UMN,UCLA, and Harvard, HCA and HCD data all in one single file under Behavioral Data - allsites . This is the 'database' for PennCNP
assessments = {
    'PennCNP': {
...
def main():
    for item in assessments:
        ...
        # Get all rows from all site output files for cleanest files as a pandas dataframe with column labels
        ...
        rowsofinterest=rows[['datasetid','siteid','subid','assessment']].copy()
        rowsofinterest['subject']=rowsofinterest.subid.str.strip()
        combowredcap=pd.merge(rowsofinterest, studyids, how='left',on='subject')  
        combonotinredcap=combowredcap.loc[combowredcap.Subject_ID.isnull()==True].copy() #these are the ids that need to be checked by the sites
        ...
        combonotinbox=combowredcap2.loc[combowredcap2.subid.isnull()==True]
        ...
        #this code needs to be moved to box.py next iteration - exclude subjects who are too young to have had the test
        datevar='interview_date'
        notinboxunique['nb_months']=(12 * (pd.to_datetime(notinboxunique[datevar]).dt.year - pd.to_datetime(notinboxunique.dob).dt.year) + 
           (pd.to_datetime(notinboxunique[datevar]).dt.month - pd.to_datetime(notinboxunique.dob).dt.month)  +
           (pd.to_datetime(notinboxunique[datevar]).dt.day - pd.to_datetime(notinboxunique.dob).dt.day)/31)
        ...
        perm_misslist=['HCA6012744','HCA6605569','HCA8689410','HCA8735289','HCA8917700','HCA9695713','HCD1539759','HCD1714852','HCD1769978','HCD2642858','HCD2913459','HCD2978386']
        notinboxunique=notinboxunique.loc[~(notinboxunique.subject.isin(perm_misslist))]
        #subset to ids listed as data_status=2, misscat=0 - these are the folks that should be 
        notinboxunique=notinboxunique.loc[notinboxunique.interview_date<'2019-08-13']
        notinboxunique=notinboxunique.loc[(notinboxunique.data_status==2) & (notinboxunique.misscat==0)]

        notinboxunique['reason']='Missing in Box'
        #####################
        ##now find duplicates and missing visits
        dupls=rows.loc[rows.duplicated(subset=['subid','assessment'],keep=False)==True][['datasetid','siteid','subid','assessment','sex','age','handedness']]
        dupls['reason']='duplicated subid/assessment combination'
        checkvisit=rows.loc[rows.assessment.isin(['V1','V2','V3'])==False][['datasetid','siteid','subid','assessment','sex','age','handedness']]
        checkvisit['reason']='visit number not in V1, V2, or V3'

        catQC=pd.concat([combonotinredcap,notinboxunique,dupls,checkvisit],axis=0,sort=True)[['datasetid','siteid','subid','assessment','subject','study','site','gender','interview_date','reason']]
        ...
        #these are the ids that need to be excluded before any data is shared...make note.
        ...
        slimhandle = assessments[item]['slim_id'] #this is a temporary file in the Behavioral Data - Snapshots / PENN / temp folder for use viewing data
...
def makeslim(storefilename,slim_id):
    ...
    combined_fileout =  os.path.join(box.cache,snapname.split('.')[0]+'Slim.csv')
