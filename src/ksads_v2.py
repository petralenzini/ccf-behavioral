...
store_space = os.path.join(root_store, 'ksads') #this will be the place to save any snapshots on the nrg servers
try:
...
def main():
    for item in assessments:
        ...
        # create snapshot of combined file (all four sites together) store snapshot in 'store' and in 'snapshots' under all sites directory in box.
        ...
        combonotinbox=combowredcap2.loc[combowredcap2.ID.isnull()==True]
        ...
        notinboxunique=notinboxunique.loc[~(notinboxunique.study=='hcpa')].copy()
        notinboxunique['reason']='Missing in Box'

        dups=combowredcap.loc[combowredcap.duplicated(subset=['PatientID','PatientType'],keep=False)]
        dups['reason']='Duplicated ID'

        catQC=pd.concat([combonotinredcap,notinboxunique,dups],axis=0,sort=True)[['ID','PatientID','subject','study','site','gender','interview_date','reason']]
        ...
        comboflaggedinredcap=combowredcap.loc[combowredcap.flagged.isnull()==False] #these are the ids that need to be
...
def share_ksads(snapshot=snapshotdate, specialstring='_20190813_LS_datarequest',boxoutdir=84470235182):
    """
    ...
    for item in assessments:
        ...
        combowredcap=combowredcap.loc[combowredcap.PatientID.str.contains('V1')==True].copy()
        combonotflaggedinredcap=combowredcap.loc[combowredcap.flagged.isnull()==True] #these are the ones that need to be excluded
        temporaryfile=os.path.join(cache_space,snapshort+specialstring+'.csv')
