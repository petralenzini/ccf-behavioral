...
# They will be repeated here (even though there is no new data) so that everything from start to finish is in one place.
...
# and from local machine ...scp -r /home/petra/endpointmachine/lifespan/* hcpinternal@hcpi-dev-petra1.nrg.mir:/data/intradb/tmp/box2nda_cache/endpointmachine/lifespan/.
...
# I.e. NO -- cat endpointmachine/lifespan/*Assessment\ Scores.csv_* | grep -v PIN > endpointmachine/AssessmentScores.csv
...
cache_space = os.path.join(root_cache, 'endpointmachine/lifespan') #dont delete cache at the end of this program until endpoint machine is back up and running
...
#just keep first of this record and note that interview date is first v1 date observed in data
...
#note that this dir has been whittled down to the files with 2019 in their file name to keep it smaller - rsync again and get everything
...
# STEP 1: get cleanest data so far and merge with Redcap to get site and study info.  Replace UMN ids, where applicable, and correct any other known (historical) issues.
...
cleanestscores=pd.read_csv(scores_path,header=0,low_memory=False, encoding = 'ISO-8859-1')
...
cleanestscores=extend_date(cleanestscores.drop(columns='FirstDate4PIN'),'DateFinished').copy()

#replace the bad UMN ids before merging with redcap - otherwise wont find a match
replacements=pd.read_csv(os.path.join(store_space,'UMN_Subject_ID_replacement_list.csv'),header=0)
...
# may need to revisit this issue to align with redcap where such duplicates were merged into one id.
replacements2=pd.read_csv(os.path.join(store_space,'HCD_to_HCA_multipleIDs.csv'),header=0)
...
for (oldid,newid) in IDmap2.items():
    ...
    if 'HCD' in newid:
        count=count+1;
        cleanestscores.loc[cleanestscores.subject == newid, 'study'] = 'HCD' #two folks for whom the multiple IDs was not an HCA to HCD issue

print(count)  #should be 2

#also remove deletions and fix typosfrom last round of QC.
replacements3=pd.read_csv(os.path.join(store_space,'HCA_HCD_whoami_06_27_2019_fixed.csv'),header=0)
droplist=replacements3.loc[replacements3.delete==1,'subject']
df=pd.merge(cleanestscores, droplist, on=['subject'], how="outer", indicator=True)
...
slimcurated=cleanestscores.drop(columns=['flagged','gender','v1_interview_date','site']).copy()
# drop study var from redcap data because dont want that level of info now
cleanestscoresmore=pd.merge(moredata.drop(columns='study'),slimcurated,how='right',on='subject')
#drop empty columns that were produced by excel during hand manipulations
cleanestscoresmore=cleanestscoresmore.drop(columns=['Unnamed: 53','Unnamed: 54', 'Unnamed: 55', 'Unnamed: 56','_merge'])
#drop anything that has endpointmachine as its source -- endpoint machine data will be reincorporated in step2
cleanestscoresmore=cleanestscoresmore.loc[cleanestscoresmore.source.str.contains("endpointmachine/")==False].copy()
...
#to find missing IDs, need to look at catscores without the inner merge with moredata
...
#note that next time around, this can all be put together (all sites collected from same catfromdate)
...
#still might be old records in here even though filtered local copy of directory by date in filename...need to filter out.
subset4scoresOthersites=catscoresmore.loc[~(catscoresmore.site=='4')]
subset4scoresOthersites=subset4scoresOthersites.loc[subset4scoresOthersites.FirstDate4PIN > '2018-12-31']

#put them together and rename a few variables in prep for catting with the cleanest box data
...
catscoresnew=catscoresnew.drop(columns=['Age-Corrected Standard Scores Quinine Whole','Age-Corrected Standard Scores Salt Whole','Uncorrected Standard Scores Quinine Whole',
       'Uncorrected Standard Scores Salt Whole', 'Whole Mouth Quinine', 'Whole Mouth Salt','Fully-Corrected T-scores Quinine Whole','Fully-Corrected T-scores Salt Whole'])
...
catscoresnew.columns.sort_values().values==cleanestscoresmore.columns.sort_values().values #should all be True - otherwise dirty data...please investigate
curatedwithcat=pd.concat([cleanestscoresmore,catscoresnew],axis=0,sort=False,verify_integrity=False)
#reorder columns
...
#lights on
#get the ones that dont have site info (indicating they still couldnt be mapped to redcap for one reason or another.
whoami=pd.DataFrame(curatedwithcat.loc[curatedwithcat.site.isnull()==True,'subject'].unique()) #74 of these
whoami.to_csv(os.path.join(store_space, 'Homeless_IDs_Toolbox_Scored_Combined_'+snapshotdate+'.csv'))
#to do: 
...
HCAorBoth=curatedwithcatreordered.loc[~(curatedwithcatreordered.study=='HCD')] #NOT (notice ~) HCD data
HCDonly=curatedwithcatreordered.loc[curatedwithcatreordered.study=='HCD'] #HCD data

WashU_HCAorBoth=HCAorBoth.loc[HCAorBoth.site=='4'].copy()
WashU_HCAorBoth_storefile=os.path.join(store_space, 'WashU_HCAorBoth_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
...
WashU_HCDonly_storefile=os.path.join(store_space, 'WashU_HCDonly_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
...
UCLA_HCAorBoth_storefile=os.path.join(store_space, 'UCLA_HCAorBoth_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
...
UCLA_HCDonly_storefile=os.path.join(store_space, 'UCLA_HCDonly_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
...
UMN_HCAorBoth_storefile=os.path.join(store_space, 'UMN_HCAorBoth_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
...
UMN_HCDonly_storefile=os.path.join(store_space, 'UMN_HCDonly_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
...
MGH_HCAorBoth_storefile=os.path.join(store_space, 'MGH_HCAorBoth_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
...
Harvard_HCDonly_storefile=os.path.join(store_space, 'Harvard_HCDonly_Toolbox_Scored_Combined_'+snapshotdate+'.csv')
...
cleanestraw=pd.read_csv(data_path,header=0,low_memory=False, encoding = 'ISO-8859-1')
...
for (oldid,newid) in IDmap2.items():
    ...
    if 'HCD' in newid:
        count=count+1;
        cleanestraw.loc[cleanestraw.subject == newid, 'study'] = 'HCD' #two folks for whom the multiple IDs was not an HCA to HCD issue
...
replacements3=pd.read_csv(os.path.join(store_space,'HCA_HCD_whoami_06_27_2019_fixed.csv'),header=0)
droplist=replacements3.loc[replacements3.delete==1,'subject']
df=pd.merge(cleanestraw, droplist, on=['subject'], how="outer", indicator=True)
...
# drop study var from redcap data because dont want that level of info now
cleanestrawmore=pd.merge(moredata.drop(columns='study'),slimcurated,how='right',on='subject')
##drop empty columns that were produced by excel during hand manipulations
cleanestrawmore=cleanestrawmore.drop(columns=['_merge'])
#drop anything that has endpointmachine as its source -- endpoint machine data will be reincorporated in step2
cleanestrawmore=cleanestrawmore.loc[cleanestrawmore.Source.str.contains("endpointmachine/")==False].copy()

##################################
#STEP2: get endpoint machine data, cat IT with output of step1
#get filenames with Data in their titles and read files into dataframe
rawdata2cat=files2cat.loc[files2cat.fname.str.contains('Assessment') & ~(files2cat.fname.str.contains('Scores'))]
...
#merge with basic redcap data so you can track down site, study, and v1 interview date
#this will only add records that have ID in REDcap
#to find missing IDs, need to look at catraw without the inner merge with moredata
...
# STEP 3: : merge the data from steps1 and steps2 and then split the result into site/subject csv files and upload to Box-site folders in box.
# merge with new data from endpoint
# after renaming a couple of things, confirm that columns are same because they should be the same
cleanestrawmoretest=cleanestrawmore.rename(columns={'Source':'source'})

catrawnew.columns.sort_values().values==cleanestrawmore.columns.sort_values().values #should all be True - otherwise dirty data...please investigate
curatedwithcat=pd.concat([cleanestrawmore,catrawnew],axis=0,sort=False,verify_integrity=False)
#reorder columns
cols=curatedwithcat.columns.tolist()
cols = cols[-1:] + cols[:-1]
...
RawHCAorBoth=curatedwithcatreorderedraw.loc[~(curatedwithcatreorderedraw.study=='HCD')] #NOT (notice ~) HCD data
RawHCDonly=curatedwithcatreorderedraw.loc[curatedwithcatreorderedraw.study=='HCD'] #HCD data

WashU_HCAorBoth=RawHCAorBoth.loc[RawHCAorBoth.site=='4'].copy()
WashU_HCAorBoth_storefile=os.path.join(store_space, 'WashU_HCAorBoth_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
WashU_HCAorBoth.to_csv(WashU_HCAorBoth_storefile,index=False)
box.upload_file(WashU_HCAorBoth_storefile,82804729845)

WashU_HCDonly=RawHCDonly.loc[RawHCDonly.site=='4'].copy()
WashU_HCDonly_storefile=os.path.join(store_space, 'WashU_HCDonly_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
WashU_HCDonly.to_csv(WashU_HCDonly_storefile,index=False)
box.upload_file(WashU_HCDonly_storefile,82804015457)

UCLA_HCAorBoth=RawHCAorBoth.loc[RawHCAorBoth.site=='2'].copy()
UCLA_HCAorBoth_storefile=os.path.join(store_space, 'UCLA_HCAorBoth_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
UCLA_HCAorBoth.to_csv(UCLA_HCAorBoth_storefile,index=False)
box.upload_file(UCLA_HCAorBoth_storefile,82807223120)

UCLA_HCDonly=RawHCDonly.loc[RawHCDonly.site=='2'].copy()
UCLA_HCDonly_storefile=os.path.join(store_space, 'UCLA_HCDonly_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
UCLA_HCDonly.to_csv(UCLA_HCDonly_storefile,index=False)
box.upload_file(UCLA_HCDonly_storefile,82805124019)

UMN_HCAorBoth=RawHCAorBoth.loc[RawHCAorBoth.site=='3'].copy()
UMN_HCAorBoth_storefile=os.path.join(store_space, 'UMN_HCAorBoth_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
UMN_HCAorBoth.to_csv(UMN_HCAorBoth_storefile,index=False)
box.upload_file(UMN_HCAorBoth_storefile,82803665867)

UMN_HCDonly=RawHCDonly.loc[RawHCDonly.site=='3'].copy()
UMN_HCDonly_storefile=os.path.join(store_space, 'UMN_HCDonly_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
UMN_HCDonly.to_csv(UMN_HCDonly_storefile,index=False)
box.upload_file(UMN_HCDonly_storefile,82805151056)

MGH_HCAorBoth=RawHCAorBoth.loc[RawHCAorBoth.site=='1'].copy()
MGH_HCAorBoth_storefile=os.path.join(store_space, 'MGH_HCAorBoth_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
MGH_HCAorBoth.to_csv(MGH_HCAorBoth_storefile,index=False)
box.upload_file(MGH_HCAorBoth_storefile,82761770877)

Harvard_HCDonly=RawHCDonly.loc[RawHCDonly.site=='1'].copy()
Harvard_HCDonly_storefile=os.path.join(store_space, 'Harvard_HCDonly_Toolbox_Raw_Combined_'+snapshotdate+'.csv')
Harvard_HCDonly.to_csv(Harvard_HCDonly_storefile,index=False)
box.upload_file(Harvard_HCDonly_storefile,82803734267)
...
def extend_date(df,datevar): #dataframe to be extended, and name of datavar on which to extend
    try:
        ...
        dates=dates.drop_duplicates(subset='PIN',keep='first')[['PIN',datevar]]
