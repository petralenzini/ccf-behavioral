...
def get_all_rows(study, fieldlist=[], subjectOnly=False):
    s = red[study]
    fieldnames = s['fields']
    token = s['token']
    events = [s['events']]
    ...
    df = redcapApi(token, fields, events)
...
class Redcap:
    ...
    def getredcapdata(self, fulldata=False):
        ...
        return pd.concat([get_all_rows(name) for name in fields_dict], sort=True)
    ...
    def getredcapfields(self, fieldlist, study):
        ...
        or elsewwhere shared
        """
        studydata = get_all_rows(study, fieldlist)
    ...
    def getredcapids(self):
        ...
        for name in red.keys():
            df = get_all_rows(name, subjectOnly=True)
