...
#get one of the identical key files which contain the labels for the numbered questions for all KSADS assessments
...
#make humanreadable data dictionary that has all the columns necessary to map (by hand) to NDA;
...
#if number of unique values in a given column is less than 10 (and thats not just because the question was only answered 10 times, print out the list ov values...otherwise, print out the first value as an example of the column contents;
for item in ksadsraw.columns:
   row=ksadsraw.groupby(item).count().reset_index()[item]
   varvalues.loc[varvalues.variable==item,'numunique']=len(row) #number of unique items in this column
   varvalues.loc[(varvalues.variable==item) & (varvalues.numunique<=10) & (varvalues.num_nonmissing>=10),'values_or_example']=''.join(str(ksadsraw[item].unique().tolist()))
   varvalues.loc[(varvalues.variable==item) & (varvalues.numunique<=10) & (varvalues.num_nonmissing<10),'values_or_example']=ksadsraw[item].unique().tolist()[1]
   varvalues.loc[(varvalues.variable==item) & (varvalues.numunique>10),'values_or_example']=ksadsraw[item].unique().tolist()[1]
