...
notinboxunique=notinboxunique.loc[notinboxunique.flagged.isnull()==True].copy()
...
missvis=combowredcap.loc[combowredcap.visit.isnull()==True]
missvis['reason']='Missing Visit Number'
#finddups
try:
    dups=pd.concat(g for _, g in baseclean.groupby("subject") if len(g) > 1) # with duplicate filenames
    dups['reason']='duplicate id'
except:
    dups=pd.DataFrame()


catQC=pd.concat([combonotinredcap,notinboxunique,missvis,dups],axis=0,sort=True)[['subject','study','site','source','gender','interview_date','reason']]
...
QCfile=os.path.join(store_space,'QC_Eprime_snapshot_'+snapshotdate+'.csv')
