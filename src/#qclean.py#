...
# Get all rows from all site output files for cleanest files as a pandas dataframe with column labels BY ASSESSMENT and tease out vars
...
baseclean=pd.read_excel(basecachefile,sheet_name='HCA-HCD_Allsites_QandRAVLT_2019',header=0) 
...
for item in asslist.assessment:
    ...
    vars()[item+'vars']=pd.read_csv(store_space+'/'+item+'_vars.csv',header=None,names=['Varnames'])
    vars()[item+'expanded']=vars()[item+'db'].row.str.split(pat=',')    
    vars()[item+'db2']=pd.DataFrame(vars()[item+'expanded'].values.tolist())
...
RAVLTdb2.loc[RAVLTdb2.delay_completion.astype(float)>1800,'delay_completion']=None

RAVLT2db2.loc[RAVLT2db2.delay_completion=='null','delay_completion']=None
RAVLT2db2.loc[RAVLT2db2.delay_completion.astype(float)>1800,'delay_completion']=None
        
WAISdb2.loc[WAISdb2.matrix_completion=='null','matrix_completion']=None
WAISdb2.loc[WAISdb2.matrix_completion.astype(float)>1800,'matrix_completion']=None
        
WISCdb2.loc[WISCdb2.matrix_completion=='null','matrix_completion']=None
WISCdb2.loc[WISCdb2.matrix_completion.astype(float)>1800,'matrix_completion']=None
        
WPPSIdb2.loc[WPPSIdb2.matrix_completion=='null','matrix_completion']=None
WPPSIdb2.loc[WPPSIdb2.matrix_completion.astype(float)>1800,'matrix_completion']=None
...
for item in ass2:
    vars()[item+'snap']=pd.concat([vars()[item+'db'],vars()[item+'db2']],axis=1)
    # create snapshot of combined file store snapshot in 'store' and in 'snapshots' under all sites directory in box.
...
for item in ass2:
    #compare ids from snapshot (currently loaded into 'rows' dataframe) with those in Redcap.
    rowsofinterest=vars()[item+'snap'][['subject','source','assessment']].copy()
    combowredcap=pd.merge(rowsofinterest, studyids, how='left',on='subject')  
    combonotinredcap=combowredcap.loc[combowredcap.Subject_ID.isnull()==True].copy() #these are the ids that need to be checked
    combonotinredcap['reason']='PatientID not in Redcap'
    notinredcap=pd.concat([notinredcap,combonotinredcap],axis=0)
    allrowsofinterest=pd.concat([allrowsofinterest,rowsofinterest],axis=0,sort=True)

combowredcap2=pd.merge(allrowsofinterest, studydata, how='right',on='subject')  
combonotinbox=combowredcap2.loc[combowredcap2.source.isnull()==True].copy()
...
notinboxunique=notinboxunique.loc[notinboxunique.flagged.isnull()==True]
...
tnotinboxunique=pd.merge(notinboxunique,status1,how='left',on='subject_id')
status2=box.getredcapfields(['data_status','misscat'],study='hcpa')
status2=status2[['data_status','misscat___7','subject_id']].copy()
tnotinboxunique=pd.merge(tnotinboxunique,status2,how='left',on='subject_id')
tnotinboxunique['misscat']=tnotinboxunique['misscat___9']
tnotinboxunique.loc[tnotinboxunique.misscat.isnull()==True,'misscat']=tnotinboxunique['misscat___7']
tnotinboxunique.loc[tnotinboxunique.data_status_x.isnull()==True,'data_status_x']=tnotinboxunique.data_status_y
status3=box.getredcapfields(['data_status','misscat'],study='hcpd18')
status3=status3[['data_status','subject_id','misscat___9']].copy()
tnotinboxunique=pd.merge(tnotinboxunique.drop(columns={'data_status_y','misscat___7','misscat___9'}),status3,how='left',on='subject_id')
tnotinboxunique.loc[tnotinboxunique.data_status_x.isnull()==True,'data_status_x']=tnotinboxunique.data_status
tnotinboxunique.loc[tnotinboxunique.misscat.isnull()==True,'misscat']=tnotinboxunique['misscat___9']
notinboxunique=tnotinboxunique.drop(columns={'data_status','misscat___9'}).rename(columns={'data_status_x':'data_status'})

notinboxunique.loc[notinboxunique.data_status=='','reason']='Missing in Box - visit summary incomplete'
notinboxunique.loc[notinboxunique.data_status=='1','reason']='Missing in Box - visit summary says complete '
notinboxunique.loc[(notinboxunique.data_status=='2') & (notinboxunique.misscat!='1'),'reason']='Missing in Box - visit summary says incomplete but cog testing not specified '
notinboxunique=notinboxunique.loc[~((notinboxunique.data_status=='2') & (notinboxunique.misscat=='1'))]

#get list of ids that need visit numbers associated with files
needsvisit=baseclean.loc[baseclean.visit.isnull()==True]
needsvisit=pd.merge(needsvisit,studydata,how='left',on='subject').drop(columns={'dob','select_4clean','flagged','gender','subject_id'})
...
catQC=pd.concat([notinredcap,notinboxunique,needsvisit],axis=0,sort=True)[['subject','interview_date','study','site','source','filename','row','reason','visit']]
...
QCfile=os.path.join(store_space,'QC_Qinteractive_assessments_snapshot_'+snapshotdate+'.csv')
catQC.to_csv(QCfile,index=False)
box.upload_file(QCfile,snapshotQCfolder)
#1, MGH (Massachusetts General Hospital) or Harvard | 2, UCLA | 3, University of Minnesota | 4, Washington University in St. Louis
...
def makedatadict(slimf,dict_id=None,cachekeyfile=None,sheet=None,folderout=None):    
    """
    create datadictionary from csvfile and upload dictionary to box
    """
    try: dictf=box.download_file(dict_id)
    except: dictf=None
    try: cachefile=os.path.join(box.cache,slimf.get().name.split('.')[0])
    except: cachefile=os.path.join(box.cache,slimf.split('.')[0]) 
    ...
    for var in ksadsraw.columns:
        ...
        try: varvalues.loc[(varvalues.variable==var) & (varvalues.numunique<=10) & 
            (varvalues.num_nonmissing<10),'values_or_example']=ksadsraw[var].unique().tolist()[1]
        except: pass
        try: varvalues.loc[(varvalues.variable==var) & (varvalues.numunique>10),'values_or_example']=ksadsraw[var].unique().tolist()[1]
        except: pass
