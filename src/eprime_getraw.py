...
processed_file = os.path.join(store_space,'ProcessedBoxFiles_AllRawData_Eprime.csv')
...
#iterate through the folders in superfolderlist and grab the text files therein
subjectfiles,subjectfolders=folderlistcontents(list(superfolderlist.foldername),list(map(int,superfolderlist.folder_id)))

#just want the text files for processing
textfiles=subjectfiles.loc[subjectfiles.filename.str.contains('.txt')==True]
...
box.download_files(files4process.file_id) #only a handful of really young kids ran this battery
#one file was thwarting naming convention and goofing up read row functions...figure out how to be more flexible....

#need to convert the windows text file to unix (extra iconv step because no BOM)
for file in files4process.filename:
    myCmd='iconv -f UTF-16 -t UTF-8 '+cache_space+'/'+file+' | dos2unix > '+cache_space+'/Utf8_unix_'+file
...
for file in files4process.filename:
    ...
    try: 
       ...
       test4=test3.loc[test3.varname.str.contains('SV')==True]
...
newprocessed_reorder=newprocessed[['file_id','filename','source','visit','subject','SV1wk20','SV2wk20','SV1mo20','SV6mo20','SV1yr20','SV3yr20','SV1wk100','SV2wk100','SV1mo100','SV6mo100','SV1yr100','SV3yr100','raw_processed_date']]


#check for dups
dups=newprocessed.loc[newprocessed_reorder.duplicated(subset=['subject','visit'],keep=False)]
...
#box.upload_file(processed_file,82670454492)# first run had to upload file - subsequent runs just update
