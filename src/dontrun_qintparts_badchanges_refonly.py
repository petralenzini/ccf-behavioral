...
#this 'download' is then saved to store space and to box raw folder under snapshots
...
processed_file = os.path.join(store_space,'ProcessedBoxFiles_AllRawData_Qinteractive.csv')
...
specialfolder2=83367512058 # this is subfolder with rescored data from late July, 2019
...
if (superfolderlist.shape[0]==2) & (superfolderlist.folder_id[0]==str(specialfolder2)) & (superfolderlist.folder_id[1]==str(specialfolder)):
    print('found expected subfolders ',superfolderlist.folder_id[0]+' and '+str(specialfolder2))
else:
    print('well, hello, new folders whose contents may or may not be captured')
    superfolderlist

#known subfolder contents -reps Cindy's initial download to UMN site folder - special folder has list of other  individual folder_ids, and 2 files...grab them
...
#second specialfolder contains rescored corrections from UMN issue circa July, 2019 email exchange.
#requires hand fixing to fix times even after getting these new files aboard. 
specialcorrectfilelist,specialcorrectfolderlist=foldercontents(specialfolder2) #grabbing df of names and id of subfolders in specialfolder
superfilelist=superfilelist.append(specialcorrectfilelist) #there were two extra files here



#known sub-sub folder contents
specialsuperfolderlist.reset_index(inplace=True)
subspecialsuperfilelist,subspecialsuperfolderlist=folderlistcontents(specialsuperfolderlist.foldername,specialsuperfolderlist.folder_id)  #4 more files,no more folders (5/22/2019)
...
superfilelist.loc[~(superfilelist.subject.str[:1]=='H')] #filenames that dont begin with H wont produce subject ids that begin with H...i.e. they dont follow naming convention.
# one was in the UCLA aging folder (9584097_5_). the other was in box-site folder but checklist said to find data in BDAS --no idea who this is  
...
dupfilenames=pd.concat(g for _, g in allboxfiles.groupby("filename") if len(g) > 1) # with duplicate filenames
...
files4process=files4process.loc[files4process.raw_processed_date.isnull()==True].copy()
...
box.download_files(files4process.file_id) #should end up with 3504 files plus whatever meta files I've made.  --note if not exact, its okay.  will grab in next run or in next steps

#for filenames with pattern in their title, file type is easy.
#determine filetype for others by looking for pattern in actual downloaded file
...
if not foundlist.empty:
    files4process=pd.merge(foundlist,files4process,on='filename',how='right')
    files4process.loc[files4process.assessment_x.isnull()==False,'assessment']=files4process.assessment_x
    files4process.loc[files4process.assessment_x.isnull()==True,'assessment']=files4process.assessment_y
    files4process=files4process.drop(columns=['assessment_x','assessment_y']).copy()
else:
    pass  #file is probably corrupted, or already has assessment type -
    #files4process.loc[files4process.filename=='HCD0040113V1.csv','raw_processed_date']=snapshotdate  #this is corrupted file

#check
singles=pd.concat(g for _, g in files4process.groupby("subject") if len(g) == 1)  #some subjects have data in only one location--yay!
duplicates=pd.concat(g for _, g in files4process.groupby("subject") if len(g) > 1)  #many subjects have data in more than one location
dupfilenames=pd.concat(g for _, g in files4process.groupby("filename") if len(g) > 1) #some even have exactly the same filenames in two locations 

#make rows of all these files, where possible because assessment is specified
#then turn the entire thing into a csv file and save in store and in box as a 'raw data' snapshot
...
for i in range(len(files4process.file_id)):
    ...
    try:
        rows,labels = create_row(cache_space,files4process.filename[i],files4process.assessment[i]) #returns row=string of comma sep values in the file, and labels=list of the labels associated with each value)
    except:
        rows=' '
...
#box.upload_file(processed_file,76432368853) first run had to upload file - subsequent runs just update
